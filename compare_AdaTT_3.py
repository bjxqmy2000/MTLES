...
class AdaTTFusionUnit(nn.Module):
    """
    AdaTT的融合单元，包含任务特定的专家和共享专家
    """

    def __init__(self, num_tasks, input_dim, num_task_experts, num_shared_experts=0,
                 expert_hidden_units=None, gate_hidden_units=None):
        super(AdaTTFusionUnit, self).__init__()
        ...
        # 每个任务的门控网络 - 组合所有专家
        total_experts = num_tasks * num_task_experts + num_shared_experts
        self.gates = nn.ModuleList([
            MLP_Block(
                input_dim=input_dim,
                output_dim=total_experts,
        ...
        # 任务特定专家的线性融合权重 - 每个任务一组权重
        self.native_weights = nn.ParameterList([
            nn.Parameter(torch.ones(num_task_experts) / num_task_experts)  # 平均初始化
            for _ in range(num_tasks)
        ])
    ...
    def forward(self, task_inputs):
        """
        前向传播
        task_inputs: list of tensors, len(task_inputs) == num_tasks
        """
        batch_size = task_inputs[0].size(0)
        expert_output_dim = self.expert_hidden_units[-1]
        ...
        for task_id in range(self.num_tasks):
            task_experts_output = []
            for expert_id in range(self.num_task_experts):
                expert_output = self.task_experts[task_id][expert_id](task_inputs[task_id])
                task_experts_output.append(expert_output)
                all_expert_outputs.append(expert_output)
            task_specific_outputs.append(task_experts_output)

        # 计算共享专家的输出 (如果存在)
        shared_outputs = []
        if self.num_shared_experts > 0:
            # 计算平均输入用于共享专家
            avg_input = torch.stack(task_inputs, dim=0).mean(dim=0)
            for expert_id in range(self.num_shared_experts):
                shared_output = self.shared_experts[expert_id](avg_input)
                shared_outputs.append(shared_output)
                all_expert_outputs.append(shared_output)

        # 将所有专家输出堆叠到一个张量中
        all_experts_stacked = torch.stack(all_expert_outputs, dim=1)  # [batch_size, total_experts, expert_output_dim]

        # 为每个任务计算融合输出
        task_outputs = []
        for task_id in range(self.num_tasks):
            # 1. 原生专家线性融合 (NativeExpertLF)
            native_linear_fusion = torch.zeros(batch_size, expert_output_dim, device=task_inputs[0].device)
            for i, expert_output in enumerate(task_specific_outputs[task_id]):
                weight = self.native_weights[task_id][i]
                native_linear_fusion += weight * expert_output

            # 2. 全专家门控融合 (AllExpertGF)
            # 计算门控权重
            gate = self.gate_activation(self.gates[task_id](task_inputs[task_id]))  # [batch_size, total_experts]
            # 应用门控机制
            all_expert_fusion = torch.bmm(gate.unsqueeze(1), all_experts_stacked).squeeze(
                1)  # [batch_size, expert_output_dim]

            # 3. 残差机制: 组合两种融合结果
            task_output = native_linear_fusion + all_expert_fusion
            task_outputs.append(task_output)

        return task_outputs


class AdaTTModel(nn.Module):
    """AdaTT模型：自适应任务到任务融合网络"""
    ...
    def __init__(self, user_feature_info, ws_feature_info, embedding_dim=16,
        ...
        super(AdaTTModel, self).__init__()
        ...
        # 服务特征嵌入 - 仅使用共享嵌入
        self.ws_embeddings = nn.ModuleDict({
        ...
        for i in range(num_layers):
            fusion_layer = AdaTTFusionUnit(
                num_tasks=self.num_tasks,
                input_dim=current_dim,
                num_task_experts=num_specific_experts,
            ...
            # 更新下一层的输入维度
            current_dim = expert_hidden_units[-1]
    ...
    def get_embeddings(self, user_features, ws_features):
        """获取特征嵌入表示"""
        # 用户特征嵌入
        user_id_emb = self.user_embeddings['user_id'](user_features[:, 0].long())
        user_ip_address_emb = self.user_embeddings['ip_address'](user_features[:, 1].long())
        user_country_emb = self.user_embeddings['country'](user_features[:, 2].long())
        user_ip_number_emb = self.user_embeddings['ip_number'](user_features[:, 3].long())
        user_as_emb = self.user_embeddings['as'](user_features[:, 4].long())
        user_latitude_emb = self.user_embeddings['latitude'](user_features[:, 5].long())
        user_longitude_emb = self.user_embeddings['longitude'](user_features[:, 6].long())

        # 服务特征嵌入
        ws_id_emb = self.ws_embeddings['ws_id'](ws_features[:, 0].long())
        ws_wsdl_address_emb = self.ws_embeddings['wsdl_address'](ws_features[:, 1].long())
        ws_provider_emb = self.ws_embeddings['provider'](ws_features[:, 2].long())
        ws_ip_address_emb = self.ws_embeddings['ip_address'](ws_features[:, 3].long())
        ws_country_emb = self.ws_embeddings['country'](ws_features[:, 4].long())
        ws_ip_number_emb = self.ws_embeddings['ip_number'](ws_features[:, 5].long())
        ws_as_emb = self.ws_embeddings['as'](ws_features[:, 6].long())
        ws_latitude_emb = self.ws_embeddings['latitude'](ws_features[:, 7].long())
        ws_longitude_emb = self.ws_embeddings['longitude'](ws_features[:, 8].long())
        ...
        # 为每个任务复制相同的嵌入向量
        task_embeddings = [feature_emb for _ in range(self.num_tasks)]
    ...
    def forward(self, user_features, ws_features):
        """前向传播"""
        # 获取特征嵌入
        task_inputs = self.get_embeddings(user_features, ws_features)

        # 通过AdaTT融合层
        for fusion_layer in self.fusion_layers:
            task_outputs = fusion_layer(task_inputs)
            task_inputs = task_outputs

        # 任务塔预测
        rt_pred = self.towers[0](task_outputs[0])  # RT任务预测
        tp_pred = self.towers[1](task_outputs[1])  # TP任务预测

        return rt_pred.squeeze(), tp_pred.squeeze()


def train_adatt_model(model, train_loader, valid_loader, optimizer, scheduler, device,
                      num_epochs, eval_epochs=10, early_stopping_patience=10,
                      rt_mean=0, rt_std=1, tp_mean=0, tp_std=1):
    """训练AdaTT模型"""
    logger.info("开始训练AdaTT模型")
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % eval_epochs == 0:
            ...
            valid_metrics = evaluate_adatt_model(
            ...
            if math.isinf(best_metrics['rt_mae']) or math.isinf(best_metrics['tp_mae']):
                ...
                # 计算平均相对提升率
                avg_improvement = (
                                          rt_mae_improvement + rt_rmse_improvement + tp_mae_improvement + tp_rmse_improvement
                                  ) / 4
...
def evaluate_adatt_model(model, valid_loader, device, rt_mean, rt_std, tp_mean, tp_std):
    """评估AdaTT模型性能"""
    model.eval()
    criterion = nn.SmoothL1Loss()
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description='AdaTT模型用于QoS预测')
    ...
    parser.add_argument('--density', type=int, default=3, help='数据密度百分比')
    ...
    parser.add_argument('--num_layers', type=int, default=1)  # AdaTT通常使用多层融合
    ...
    # 加载数据
    logger.info(f"正在加载AdaTT数据，密度: {args.density}%...")
    dataset = Dataset(
    ...
    logger.info("===== AdaTT模型配置 =====")
    logger.info(f"嵌入维度: {args.embedding_dim}")
    logger.info(f"融合层数: {args.num_layers}")
    ...
    # 训练AdaTT模型
    best_metrics = train_adatt_model(
        model=model,
