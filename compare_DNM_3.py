...
class DNMModel(nn.Module):
    """DNM模型：Deep Neural Model for QoS Prediction"""

    def __init__(self, user_feature_info, ws_feature_info, embedding_dim=64,
                 perception_hidden_dims=[128, 64], task_specific_hidden_dims=[64, 32],
                 dropout_rate=0.2):
        super().__init__()
        ...
        # 计算嵌入后的特征维度
        self.user_embed_dim = 7 * embedding_dim  # 7个用户特征
        self.ws_embed_dim = 9 * embedding_dim  # 9个服务特征
        ...
        for hidden_dim in perception_hidden_dims:
            perception_layers.append(nn.Linear(input_dim, hidden_dim))
            perception_layers.append(nn.ReLU())
            input_dim = hidden_dim

        self.perception_layers = nn.Sequential(*perception_layers)
        self.perception_output_dim = perception_hidden_dims[-1]

        # RT任务特定层
        rt_task_layers = []
        input_dim = self.perception_output_dim
        for hidden_dim in task_specific_hidden_dims:
            rt_task_layers.append(nn.Linear(input_dim, hidden_dim))
            rt_task_layers.append(nn.ReLU())
            input_dim = hidden_dim
        rt_task_layers.append(nn.Linear(input_dim, 1))
        self.rt_task_layers = nn.Sequential(*rt_task_layers)

        # TP任务特定层
        tp_task_layers = []
        input_dim = self.perception_output_dim
        for hidden_dim in task_specific_hidden_dims:
            tp_task_layers.append(nn.Linear(input_dim, hidden_dim))
            tp_task_layers.append(nn.ReLU())
            input_dim = hidden_dim
        tp_task_layers.append(nn.Linear(input_dim, 1))
        self.tp_task_layers = nn.Sequential(*tp_task_layers)

    def get_embeddings(self, user_features, ws_features):
        """获取用户和服务的嵌入表示，直接返回堆叠的张量"""
        # 用户特征嵌入
        ...
        # 使用 stack 直接堆叠用户特征嵌入 [batch_size, 7, embedding_dim]
        ...
        # 使用 stack 直接堆叠服务特征嵌入 [batch_size, 9, embedding_dim]
        ws_embeddings = torch.stack([
        ...
        return user_embeddings, ws_embeddings

    def interaction_layer(self, user_embeddings, ws_embeddings):
        """交互层：产生一阶特征和二阶交互特征"""
        # 现在 user_embeddings 已经是 [batch_size, 7, embedding_dim]
        # ws_embeddings 已经是 [batch_size, 9, embedding_dim]

        # 一阶特征：将用户和服务的所有嵌入直接相加
        # 用户嵌入求和: [batch_size, 7, embedding_dim] -> [batch_size, embedding_dim]
        user_sum = torch.sum(user_embeddings, dim=1)
        # 服务嵌入求和: [batch_size, 9, embedding_dim] -> [batch_size, embedding_dim]
        ws_sum = torch.sum(ws_embeddings, dim=1)
        # 一阶特征
        first_order = user_sum + ws_sum  # [batch_size, embedding_dim]

        # 二阶交互特征：用户的每个嵌入和服务的每个嵌入计算元素积，再将所有结果相加
        # 扩展维度以便计算所有用户-服务嵌入对的交互
        # user_expanded: [batch_size, 7, 1, embedding_dim]
        user_expanded = user_embeddings.unsqueeze(2)
        # ws_expanded: [batch_size, 1, 9, embedding_dim]
        ws_expanded = ws_embeddings.unsqueeze(1)

        # 计算所有用户-服务嵌入对的元素积
        # interaction_pairs: [batch_size, 7, 9, embedding_dim]
        interaction_pairs = user_expanded * ws_expanded

        # 将所有交互对相加得到二阶特征
        # second_order: [batch_size, embedding_dim]
        second_order = torch.sum(interaction_pairs, dim=[1, 2])

        # 将一阶和二阶特征相加
        interaction_output = first_order + second_order  # [batch_size, embedding_dim]
    ...
    def forward(self, user_features, ws_features):
        # 1. 嵌入层
        user_embeddings, ws_embeddings = self.get_embeddings(user_features, ws_features)

        # 2. 交互层
        interaction_output = self.interaction_layer(user_embeddings, ws_embeddings)

        # 3. 感知层（共享层）
        shared_features = self.perception_layers(interaction_output)

        # 4. 任务特定层
        rt_pred = self.rt_task_layers(shared_features)
        tp_pred = self.tp_task_layers(shared_features)

        return rt_pred, tp_pred


def train_dnm_model(model, train_loader, valid_loader, optimizer, scheduler, device,
                    num_epochs, eval_epochs=10, early_stopping_patience=10,
                    rt_mean=0, rt_std=1, tp_mean=0, tp_std=1):
    """训练DNM模型"""
    logger.info("开始训练DNM模型")
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % eval_epochs == 0:
            ...
            valid_metrics = evaluate_dnm_model(
...
def evaluate_dnm_model(model, valid_loader, device, rt_mean, rt_std, tp_mean, tp_std):
    """评估DNM模型性能"""
    model.eval()
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description='DNM模型用于QoS预测')
    ...
    parser.add_argument('--perception_hidden_dims', type=str, default='512,256')
    parser.add_argument('--task_specific_hidden_dims', type=str, default='128,64')
    ...
    # 解析维度参数
    perception_hidden_dims = [int(x) for x in args.perception_hidden_dims.split(',')]
    task_specific_hidden_dims = [int(x) for x in args.task_specific_hidden_dims.split(',')]
    ...
    # 加载数据
    logger.info(f"正在加载DNM数据，密度: {args.density}%...")
    dataset = Dataset(
    ...
    logger.info(f"感知层隐藏维度: {perception_hidden_dims}")
    logger.info(f"任务特定层隐藏维度: {task_specific_hidden_dims}")
    logger.info("========================")
    ...
    model = DNMModel(
        ...
        perception_hidden_dims=perception_hidden_dims,
        task_specific_hidden_dims=task_specific_hidden_dims,
