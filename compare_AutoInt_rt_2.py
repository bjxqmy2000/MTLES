...
class InteractingLayer(nn.Module):
    ...
    def forward(self, x):
        # 自注意力 - PyTorch的MultiheadAttention返回(attn_output, attn_output_weights)
...
class AutoInt(nn.Module):
    """AutoInt模型，使用PyTorch内置的多头自注意力机制自动学习特征交互"""
    ...
    def __init__(self, user_feature_info, ws_feature_info, embedding_dim=128,
        ...
        prev_dim = self.num_features * embedding_dim
    ...
    def forward(self, user_features, ws_features):
        ...
        # 特征交互部分
        attn_output = embedded_features
        for layer in self.interacting_layers:
            attn_output = layer(attn_output)

        # 展平交互特征
        attn_output_flat = attn_output.reshape(batch_size, -1)  # [batch_size, num_features * embedding_dim]
...
# 用于RT任务的AutoInt模型训练
def train_autoint_rt_model(model, train_loader, valid_loader, criterion,
                           optimizer, scheduler, num_epochs, device,
                           early_stopping_patience=10, rt_mean=0, rt_std=1, eval_epochs=10):
    """训练RT任务的AutoInt模型"""
    logger = logging.getLogger(__name__)
    logger.info("开始训练RT任务的AutoInt模型")
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % eval_epochs == 0:  # 每k个epoch进行一次验证
            ...
            valid_metrics = evaluate_autoint_rt_model(
...
# 用于RT任务的AutoInt模型评估
def evaluate_autoint_rt_model(model, valid_loader, criterion, device, rt_mean, rt_std):
    """评估RT任务的AutoInt模型性能"""
    model.eval()
    valid_loss = 0.0
    batch_count = 0
...
# 用于TP任务的AutoInt模型训练
def train_autoint_tp_model(model, train_loader, valid_loader, criterion,
                           optimizer, scheduler, num_epochs, device,
                           early_stopping_patience=10, tp_mean=0, tp_std=1, eval_epochs=10):
    """训练TP任务的AutoInt模型"""
    logger = logging.getLogger(__name__)
    logger.info("开始训练TP任务的AutoInt模型")
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % eval_epochs == 0:  # 每k个epoch进行一次验证
            ...
            valid_metrics = evaluate_autoint_tp_model(
...
# 用于TP任务的AutoInt模型评估
def evaluate_autoint_tp_model(model, valid_loader, criterion, device, tp_mean, tp_std):
    """评估TP任务的AutoInt模型性能"""
    model.eval()
    valid_loss = 0.0
    batch_count = 0
...
def main():
    """主函数"""
    # 参数设置
    parser = argparse.ArgumentParser(description='AutoInt模型用于QoS预测')
    ...
    if args.task == 'rt_only':
        ...
        # 训练AutoInt模型
        best_metrics = train_autoint_rt_model(
            model=model,
        ...
        # 训练AutoInt模型
        best_metrics = train_autoint_tp_model(
            model=model,
            train_loader=train_loader,
            valid_loader=valid_loader,
            criterion=criterion,
