...
class SingleTaskEmbedding(nn.Module):
    ...
    def forward(self, user_features, ws_features):
        ...
        flat_features = features_stacked.reshape(batch_size, -1)  # [batch_size, num_features * embedding_dim]
...
class CrossLayer(nn.Module):
    """
    DCN模型的交叉层，实现显式特征交叉
    按照公式: x_{l+1} = x_0 * (x_l^T * w_l) + b_l + x_l
    """
    ...
    def forward(self, x0, xl):
        """
        x0: 初始输入，形状 [batch_size, input_dim]
        xl: 当前层输入，形状 [batch_size, input_dim]
        """
        # 计算 x_l^T * w_l
        xl_w = torch.matmul(xl, self.weight)  # [batch_size, 1]

        # 计算 x_0 * (x_l^T * w_l)
        # 这里使用广播机制将xl_w扩展到与x0相同的形状
        x0_xl_w = x0 * xl_w  # [batch_size, input_dim]

        # 添加偏置和残差连接
        xl_plus = x0_xl_w + self.bias + xl
...
class DCNModel(nn.Module):
    """Deep & Cross Network模型用于单任务QoS预测"""

    def __init__(self, user_feature_info, ws_feature_info, embedding_dim=128,
                 num_cross_layers=3, deep_hidden_dims=[256, 128, 64], prediction_hidden_dims=[128, 64]):
        super().__init__()
        ...
        for hidden_dim in deep_hidden_dims:
            deep_layers.append(nn.Linear(input_size, hidden_dim))
        ...
        # 预测头网络
        prediction_layers = []
        # 预测头的输入维度是交叉网络输出和深度网络输出的拼接
        pred_input_size = self.input_dim + deep_hidden_dims[-1]

        for hidden_dim in prediction_hidden_dims:
            prediction_layers.append(nn.Linear(pred_input_size, hidden_dim))
            prediction_layers.append(nn.ReLU())
            pred_input_size = hidden_dim

        # 添加最终的输出层
        prediction_layers.append(nn.Linear(pred_input_size, 1))

        self.prediction_head = nn.Sequential(*prediction_layers)

    def forward(self, user_features, ws_features):
        # 获取嵌入特征
        embeddings = self.embedding(user_features, ws_features)

        # 交叉网络
        cross_output = embeddings
        for cross_layer in self.cross_layers:
            cross_output = cross_layer(embeddings, cross_output)

        # 深度网络
        deep_output = self.deep_network(embeddings)
...
def train_dcn_rt_model(model, train_loader, valid_loader, criterion,
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % eval_epochs == 0:  # 每k个epoch进行一次验证
            ...
            valid_metrics = evaluate_dcn_rt_model(
...
def evaluate_dcn_rt_model(model, valid_loader, criterion, device, rt_mean, rt_std):
    """评估RT任务的DCN模型性能"""
    model.eval()
    valid_loss = 0.0
    batch_count = 0
...
def train_dcn_tp_model(model, train_loader, valid_loader, criterion,
                      optimizer, scheduler, num_epochs, device,
                      early_stopping_patience=10, tp_mean=0, tp_std=1, eval_epochs=10):
    """训练TP任务的DCN模型"""
    logger.info("开始训练TP任务的DCN模型")
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % eval_epochs == 0:  # 每k个epoch进行一次验证
            ...
            valid_metrics = evaluate_dcn_tp_model(
...
def evaluate_dcn_tp_model(model, valid_loader, criterion, device, tp_mean, tp_std):
    """评估TP任务的DCN模型性能"""
    model.eval()
    valid_loss = 0.0
    batch_count = 0
...
def main():
    """主函数"""
    # 参数设置
    parser = argparse.ArgumentParser(description='DCN模型用于QoS预测')
    ...
    # 任务模式
    parser.add_argument('--task', type=str, default='rt_only', choices=['rt_only', 'tp_only'])
    ...
    if args.task == 'rt_only':
        ...
        logger.info("===== DCN RT单任务模型配置 =====")
        logger.info(f"嵌入维度: {args.embedding_dim}")
        logger.info(f"交叉层数量: {args.num_cross_layers}")
        ...
        # 训练DCN模型
        best_metrics = train_dcn_rt_model(
            model=model,
        ...
        # 训练DCN模型
        best_metrics = train_dcn_tp_model(
            model=model,
            train_loader=train_loader,
            valid_loader=valid_loader,
            criterion=criterion,
