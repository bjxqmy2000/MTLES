...
# 特征嵌入层，用于单任务嵌入，基于论文中DeepFM的特性，共享嵌入
class SharedEmbedding(nn.Module):
    """特征嵌入层，同时为FM组件和Deep组件提供共享嵌入"""
...
class FMLayer(nn.Module):
    ...
    def forward(self, embedded_features):
        ...
        计算FM部分的输出
        Args:
            embedded_features: [batch_size, num_features, embedding_dim]
        Returns:
            FM输出: [batch_size, 1]
        """
        # 二阶项: 特征间的交互
        sum_features = torch.sum(embedded_features, dim=1)  # [batch_size, embedding_dim]
        sum_square = torch.sum(sum_features ** 2, dim=1, keepdim=True)  # [batch_size, 1]
        square_sum = torch.sum(torch.sum(embedded_features ** 2, dim=2), dim=1, keepdim=True)  # [batch_size, 1]
        second_order = 0.5 * (sum_square - square_sum)  # [batch_size, 1]
...
class DeepFM(nn.Module):
    """DeepFM模型，结合FM和深度神经网络"""

    def __init__(self, user_feature_info, ws_feature_info, embedding_dim=128,
                 deep_hidden_dims=[256, 128, 64], prediction_hidden_dims=[128, 64]):
        super().__init__()
        ...
        # FM组件
        self.fm = FMLayer(self.num_features, self.embedding_dim)
        ...
        prev_dim = deep_hidden_dims[-1] + 1  # FM输出维度为1，Deep输出维度为deep_hidden_dims[-1]
    ...
    def forward(self, user_features, ws_features):
        ...
        # FM部分
        fm_output = self.fm(embedded_features)  # [batch_size, 1]
        ...
        # 合并FM和Deep的输出
        combined = torch.cat([fm_output, deep_output], dim=1)  # [batch_size, deep_hidden_dims[-1] + 1]
...
def train_deepfm_rt_model(model, train_loader, valid_loader, criterion,
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % eval_epochs == 0:  # 每k个epoch进行一次验证
            ...
            valid_metrics = evaluate_deepfm_rt_model(
...
# 用于RT任务的DeepFM模型评估
def evaluate_deepfm_rt_model(model, valid_loader, criterion, device, rt_mean, rt_std):
    """评估RT任务的DeepFM模型性能"""
    model.eval()
    valid_loss = 0.0
    batch_count = 0
...
# 用于TP任务的DeepFM模型训练
def train_deepfm_tp_model(model, train_loader, valid_loader, criterion,
                          optimizer, scheduler, num_epochs, device,
                          early_stopping_patience=10, tp_mean=0, tp_std=1, eval_epochs=10):
    """训练TP任务的DeepFM模型"""
    logger = logging.getLogger(__name__)
    logger.info("开始训练TP任务的DeepFM模型")
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % eval_epochs == 0:  # 每k个epoch进行一次验证
            ...
            valid_metrics = evaluate_deepfm_tp_model(
...
# 用于TP任务的DeepFM模型评估
def evaluate_deepfm_tp_model(model, valid_loader, criterion, device, tp_mean, tp_std):
    """评估TP任务的DeepFM模型性能"""
    model.eval()
    valid_loss = 0.0
    batch_count = 0
...
def main():
    ...
    if args.task == 'rt_only':
        ...
        # 训练DeepFM模型
        best_metrics = train_deepfm_rt_model(
            model=model,
        ...
        # 训练DeepFM模型
        best_metrics = train_deepfm_tp_model(
            model=model,
            train_loader=train_loader,
            valid_loader=valid_loader,
            criterion=criterion,
